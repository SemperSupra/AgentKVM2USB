...
- Wyse BIOS automation mapping started on 2026-08-03 and is tracked in `WYSE_BIOS_AUTOMATION.md`. Video is healthy at `1920x1080 active`, and the Wyse shows `No bootable devices found` with `F1`, `F2`, and `F5` options. Live macro probes for `F2`, `F5`, `Ctrl+Alt+Delete`, and `Caps Lock` wrote valid KVM2USB keyboard reports, but the Wyse did not visibly react and LED state did not change. A post-power-cycle read-only MI_00 probe still reported `RGB 1920x1080p@60.318, HDMI`.
- Total Phase Beagle USB 12 setup is now usable on the host. The driver package `totalphaseusb-v4.0.0.zip` was installed on 2026-08-03, Windows reports `USB\VID_1679&PID_2001\TP1112-141536` as `Total Phase Beagle Protocol Analyzer` with WinUSB and problem code `0`, and the vendor API detects port `0` available with serial `1112-141536`.
- `scripts/capture_hid_path_experiment.py` now captures host-side HID descriptors, before/after status, macro write results, screen paths, and timestamped `host-log.jsonl` for correlation with an inline Beagle-12 trace on the Wyse-facing USB leg. It supports `--start-delay-ms` for analyzer synchronization.
- `scripts/capture_beagle_usb12.py` now captures Total Phase Beagle USB 12 JSONL traces through the vendor Python API, decodes USB PIDs and token address/endpoint fields, and summarizes packet, event, endpoint, and HID-data counts.
- Latest synchronized target-side capture: `.work\beagle\wyse-hid-capslock-inline-decoded.jsonl` plus `.work\experiments\wyse-hid-capslock-inline-decoded`. During a host-side Caps Lock on/off macro that returned successful 9-byte keyboard write counts, the Wyse-facing bus showed `1868` `IN` polls to USB address `23`, endpoint `2`, `1867` `NAK` handshakes, and `0` HID data packets. Interpretation: the Wyse-facing USB leg is present and polling, but KVM2USB is not forwarding host-side keyboard writes to the target-side HID endpoint in the current state. The next blocker is the missing target-side HID forwarding, activation, or re-enumeration step rather than basic cable absence.
...
- `.venv\Scripts\python.exe -m pytest -q` (`68 passed`, after Beagle USB12 capture automation)
