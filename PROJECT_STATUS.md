...
- Wyse BIOS automation mapping started on 2026-08-03 and is tracked in `WYSE_BIOS_AUTOMATION.md`. Video is healthy at `1920x1080 active`, and the Wyse shows `No bootable devices found` with `F1`, `F2`, and `F5` options. Live macro probes for `F2`, `F5`, and `Ctrl+Alt+Delete` wrote valid KVM2USB keyboard reports, but the Wyse did not visibly react. The current blocker is the target-side USB HID path, most likely the Epiphan KVM cable USB-A leg not connected to or not enumerated by the Wyse.
