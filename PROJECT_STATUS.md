...
- First-pass FPGA bitstream decoding now normalizes per-byte bit-reversal to canonical Xilinx sync `aa 99 55 66` and emits packet-like summaries. The real payload currently reports `200` packet-like records, opcode counts `nop=195`, `read=1`, `write=4`, packet type counts `0=179`, `1=16`, `2=1`, `4=4`, and one truncated interpretation. Treat this as bit-order/framing progress, not a complete Spartan-6 semantic decode.
